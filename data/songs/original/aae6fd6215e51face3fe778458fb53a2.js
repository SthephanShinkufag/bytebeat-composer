...
function dsp(t){
  ...
  var p = sequence(1/2, progression);

  var melody = [5, 17, 5, 17, 8, 20, 8, 20].map(function(n){
    return note(n + p, 0);
  });

  filter.set_fc( 0.001 + ((lfo.play(2.66) * 0.16 + 1) / 2) * (0.29 + lfo2.play(.01) * 0.17) ) ;

  var n = slide(1/16, melody, 14);

  var synth_osc = osc.play(n);
  var synth = arp(1/16, synth_osc, 50, 50);

  synth = filter.run(synth * 0.6);
  synth = clip(synth * 12);

  var kick = arp(1/4, 65, 63, 5);
