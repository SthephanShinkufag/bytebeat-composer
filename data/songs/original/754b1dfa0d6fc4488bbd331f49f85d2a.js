...
window.channels.forEach(chan => {
	...
	chan.ndx = ndx;
	note = chan.notes[ndx];
	let inc = 1/(note.end-note.start)/(SAMP_RATE/(BPM*1.5));
...
acid.process(tone.hp24),acid.lp24*ampenv.process()/2