...
o = 34,
r = (w, s, i) => (
	v++ || (a = Array(round(o * s)).fill(0)),
	w = w % 256 + a[z = v % a.length],
	a[z] = w * i,
	w
),
...
saw = w => w / 64 % 1 - .5,
tri = w => asin(sin(w * PI / 8)),

// Chord maker
c = (i, s = 0, r = 1, d = sin) => (
	l = i.map(i => p(w, i + s)),
	m = l.reduce((q, s) => d(q * r) + d(s * r) / sqrt(i.length)),
	m / i.length
),
...
b = x => x.toString(2), r(
	...
	saw(w * (0xfacab1eb01df01d5 * ((w >> 13) + 5)).toString(7)[(w >> 9) % 14] * 2 *
		p(.5, [3, -2][y >> 10 & 1])) / 2 +
