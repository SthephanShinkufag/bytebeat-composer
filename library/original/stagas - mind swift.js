...
var melody = [2, 9, 5, 8].map(function(n){
  return note(n, 1);
});

melody = melody.concat(melody.slice().reverse());

var hat_note = note(12, 5);
...
function dsp(t){
  ...
  var n = slide(1/8, melody, 12);

  var synth_osc = osc.play(n + tri(20) * 19) + tri(n / 10000);
  var synth = arp(1/16, synth_osc, 20, 10);

  filter.set_fc(0.14 + (tri(1/8) * 0.08));
  var b = slide(1/2, melody, 3);
  var bass = bass_osc.play(b / 4);
  bass = filter.run(bass * 0.4);
  bass = clip(bass * 9);
  bass = bass * (0.7 - ((saw(2) * 0.95)));
