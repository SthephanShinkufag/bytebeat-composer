//Coded by Bp103
sr=44100,q=t/sr, 
t<1? (notetbl=[],notetbl[48]=440,ramp=[],ramp[0]=0,hat=[],noise=[]):0, // workaround to make arrays work
...
function main(){
	...
	if(((q*4)%1)==0) rndnl = int(random()*10);
	if(((q*8)%1)==0) rndnr = int(random()*10);
...
function sampler(sample,place,pitch,vol){
	return (sample[int((place*(pitch*(sample[0]-1)))%((sample[0]-1)+1))]/255)*vol;
}