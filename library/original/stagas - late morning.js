...
function dsp(t){
  ...
  // bass
  var bass_osc =
    0.05 * sin(c[2 + (Math.floor(sin((1/8) * 2, t)))] * (2 + (Math.floor(tri(1/2, t) * 1))) / 5)
  + 0.07 * tri(c[2 + (Math.floor(sin((1/4) * 2, t)))] * (2 + (Math.floor(tri(1/4, t) * 1))) * 3)
  ;

  bass_osc = bass_lp(bass_osc, 30 + tri(4) * 3);

  var bass = arp(1/6, bass_osc, 2, 10 + (tri(1/4) * 20));
  ...
  var synth = plucked(arp(1/3, synth_osc, sequence(1, [1, 1, 1, 0]) * 45 + (sin(0.5) * 10), 0.3));
