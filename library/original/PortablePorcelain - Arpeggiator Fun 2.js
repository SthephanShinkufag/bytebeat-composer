...
t ? 0 : (s = Y = 0, X = 'repeat', R = sin, G = Array(12289).fill(0)),
	...
	lim = (x, P = .1) => (
		J = 0, x &= 255, V = G[J] = min(G[J] + P, x, 255), C = G[J + 1] = max(G[J + 1] - P, x, V + 9), J += 2, (x - V) * 255 / (C - V)
	),
	n = k => k == NaN ? 0 : k,
	p = (k, l, q, z = T >> 12) => q(k * (n(l = l.charCodeAt(z % l.length)) <= 32 ? 0 : 2 ** ((l - 74) / 12))),
	t2 = s / I | 0,
	K = (x, L, F, D, W) => (
		E = J + (t2 % L), x = x * D + W * G[E], G[E] = x * F, J += L, x
	),
	T /= 4,
	...
	O = {
		b: s =>
			p(s, '@@@@@@BCEEECCBCCEEEEECEEBBB@@@  ', k =>
				k / 4 % 64 + (k / 4 + t / 1600) % 64
			) * 2,
		e: p(p(s / 2 ** (1 / 6), '@' [X](18) + '@B@' + '@' [X](75), k => k, T * 3 >> 12), 'SSSSSSSQOOONNOLJLLLLLNLJIIIGGG  ', k =>
			cbrt(R(k * PI / 32)) * 45) + 32,
		a: s =>
			(p(s * 2 ** ((12 * -int(a / 4)) / 12), P, k =>
				(k / 8 & 31) * 1.5,
				a % 4 + (int(S / 8) * 4))),
		l: s =>
			p(s, P,
	...
	M = {
		p: (a, b, c) =>
			(K(L(O.b(s),
				R(t / 65536) * 4 + 8) + O.a(s) + (H > 8) * cbrt(R(cbrt(T & (H > 16 ? 8191 : 16383)) * 14)) * 32 + (H > 16) * (O.e + O.b(s * 1.5)) + (H > 24) * O.l(s) - (H > 32) * (O.a(s * 1.5) + (O.l(s * 2) + O.l(s / 2)) / 2), 12288, a, b, c) / 256 - 1.4) / (H > 16 ? 2 : 1.5)
	},
