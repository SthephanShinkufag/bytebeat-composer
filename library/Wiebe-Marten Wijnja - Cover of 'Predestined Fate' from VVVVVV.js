...
bt = (btm2 >> (t / btime) % 16) & 1 ?
  sin((PI * (t % btime) * pow(2, btm / 12 - 1))) * pow(1 - (t % btime) / btime, 10) * .3 : 0,

0 +
(v < 640 ?
  // Arpeggio
  M(6, 5, (t >> 12) % 32, m, 3) * .3 +
  M(6, 3, (t >> 12) % 32, m, 3) * .01 +
  (v < 64 ? 0 : M(6, 4, (t >> 12) % 32, m, 2) * .05) +
  // Bell
  (v < 128 ? 0 : (
    M(6, 3, (t >> 16) % 2, m2, 2) +
    M(9, 4, (t >> 16) % 2, m2, 2) +
  ...
  // First melody
  (v < 196 ? 0 : M(6, 4, (t >> 12) % 32, m3, ((t >> 17) % 2 ? 0 : 1)) * .05) +
  ...
  (v > 255 && (v < 448 || v > 511) ?
    ...
    // Second melody
    (v < 20 ? 0 :
      M(6, 3, (t >> 13) % 32, m4, 2, m4b, 0x8000) * .1 +
      M(6, 4, (t >> 13) % 32, m4, 1, m4b, 0x8000) * .05) +
    (v < 320 ? 0 : M(6, 3, (t >> 12) % 32, ((t >> 17) % 2 ? m5 : ' '), 3) * .2) : 0) :
  // Outro
  // Intermezzo melody
  M(6, 4, (t >> 13) % 32, m6, 3) * .05 +
  // Intermezzo bass
  M(6, 5, (t >> 12) % 32, m7, 2) * (1 - (t % (2 << 11)) / (2 << 11)) * .05 +
  // Distorted drum effect
  ((t >> 15) % 4 ? 0 : ((((sqrt(t % 0x2000) << 6 & 255) / 127 - 1)) / ((t >> 13) % 4 + 1)) * .15)
)